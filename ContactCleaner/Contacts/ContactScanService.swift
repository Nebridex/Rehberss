...
final class ContactScanService {
    ...
    static func snapshot(_ contact: CNContact, source: ContactSource, groups: [String], includeNote: Bool) -> ContactSnapshot {
        ContactSnapshot(
            ...
            phones: contact.phoneNumbers.map { LabeledString(label: $0.label ?? CNLabelPhoneNumberMobile, value: $0.value.stringValue) },
            emails: contact.emailAddresses.map { LabeledString(label: $0.label ?? CNLabelHome, value: String($0.value)) },
            postalAddresses: contact.postalAddresses.map {
                PostalAddressSnapshot(
                    label: $0.label ?? CNLabelHome,
            ...
            urlAddresses: contact.urlAddresses.map { LabeledString(label: $0.label ?? CNLabelURLAddressHomePage, value: String($0.value)) },
            relations: contact.contactRelations.map { LabeledString(label: $0.label ?? CNLabelOther, value: $0.value.name) },
            dates: contact.dates.map { LabeledDateSnapshot(label: $0.label ?? CNLabelDateAnniversary, components: $0.value as DateComponents) },
            socialProfiles: contact.socialProfiles.map {
                SocialProfileSnapshot(
                    label: $0.label ?? CNLabelOther,
            ...
            instantMessages: contact.instantMessageAddresses.map {
                InstantMessageSnapshot(label: $0.label ?? CNLabelOther, username: $0.value.username, service: $0.value.service)
            },
